...
yang.fun_.makeProductions = function() {
    ...
    yang.test_.test_production.push(
        ...
        new Production(
                yang.memorize_unaten_food.name,
                yang.memorize_unaten_food.priority,
                yang.memorize_unaten_food.condition,
                yang.memorize_unaten_food.action
        ),
        new Production(
 ...
 * @type {yang.fun_.def_node_construct}
 */
yang.node_.memorize_unaten_food = new yang.fun_.def_node_construct("production_node");
yang.node_.memorize_unaten_food.name = "memorize_unaten_food";
yang.node_.memorize_unaten_food.priority = Production.priority.High;
yang.node_.memorize_unaten_food.condition = function () {
    return (yang.memory_.colliding_obj instanceof Entity)
        && ((yang.memory_.colliding_obj.name.toLowerCase()).search("berry") > 0
        || (yang.memory_.colliding_obj.name.toLowerCase()).search("fruit") > 0)
        && yang["mental_task_node"] != yang.node_.id_prime_focus
...
yang.node_.memorize_unaten_food.action = function () {
